...
module.exports.init = function(webdriverInstance, options) {
    ...
    webdriverInstance.addCommand = function(name, func) {
      this[name] = function() {
    ...
    webdriverInstance.windowHandleSize = function() {
      if (arguments.length === 1) {
        var callback = arguments[0];
        this.windowSize('current', function(res) {
          callback(null, res);
        });
      } else {
        var callback = arguments[1];
        this.windowSize('current', arguments[0].width, arguments[0].height, callback);
      }
    };

    webdriverInstance.logger = console;

    webdriverInstance.call = function(callback) {
